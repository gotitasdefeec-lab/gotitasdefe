...
async function getStoreConfig() {
  try {
    const res = await fetch(`${BASE}/public/store/config`, { next: { revalidate: 86400, tags: ['store-config'] } });
...
async function getLogoUrl() {
  try {
    const res = await fetch(`${BASE}/public/store/logo`, { next: { revalidate: 86400, tags: ['store-logo'] } });
    if (!res.ok) return '';
...
async function getPolicies() {
  try {
    const res = await fetch(`${BASE}/public/policies`, { next: { revalidate: 86400, tags: ['policies'] } });
