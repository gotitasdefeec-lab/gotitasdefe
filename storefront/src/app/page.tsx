...
async function getFeaturedProducts() {
  try {
    ...
    const res = await fetch(url, { 
      next: { revalidate: 86400, tags: ['products', 'featured-products'] } // 24h cache + on-demand revalidation
    });
    if (!res.ok) {
...
async function getCarouselSlides() {
  try {
    const res = await fetch(`${BASE}/public/carousel`, { 
      next: { revalidate: 86400, tags: ['carousel'] } // 24h cache + on-demand revalidation
    });
    if (!res.ok) {
